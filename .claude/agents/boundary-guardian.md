...
- [ ] **业务包互不依赖**：`packages/{access,audit,config,observability,devboard}/package.json#dependencies` 中不含其他业务 `@gocell/*`
...
- [ ] **PDP 接线**：`<Can>` UI 壳在 `@gocell/core`，PDP client 在 `@gocell/access`；不在路由 meta 硬编 role
