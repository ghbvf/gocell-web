 ...
 *
 * 说明：'/' 首页无需登录即渲染 AppShell（骨架阶段无 auth guard）。
 * 登录重定向冒烟留 Batch 1。
 */
...
test.describe('主题切换', () => {
  test('点击主题按钮后 html[data-theme] 变化', async ({ page }) => {
    ...
    // TopBar 中主题切换按钮为 header 内的最后一个 button（aria-label 含 "dark" 或 "light"）
    // 用宽松选择器：TopBar 的 actions 区域中 aria-label 包含 dark 或 light 关键字的按钮
    const themeBtn = page.locator('header button[aria-label]').last()
